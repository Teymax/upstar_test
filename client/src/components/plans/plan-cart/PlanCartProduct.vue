    ...
    <div>
        <v-row>
            <PlansHeader
                    ...
                    @back="$emit('back')"
            />
            <p class="font__description">Your plan:</p>
        </v-row>
        <PlanCartDescription
        ...
        />
        <div v-if="tmpCurrentPlan.products.length" class="plan-cart-products py-8">
            <div
            ...
            >
                <v-row class="align-center">
                    ...
                    <v-col  sm="6" md="6" lg="6" class="plan-cart-product-description-product">
                        <p class="plan-cart-product-description-product__title font__title mb-3">{{ item.title }}</p>
                        <p class="plan-cart-product-description-product__description mb-0">{{ item.description}} </p>
                    </v-col>
                    <v-col  sm="6" md="6" lg="2">
                        <p class="plan-cart-product__price mb-0"><span
                                class="plan-cart-product__price_small mr-1">$</span>{{ item.price }}</p>
                    </v-col>
                    <v-col  sm="6" md="6" lg="2" class="counter-products d-flex align-center">
                        <v-btn
                                outlined
                                height="49"
                                min-width="49"
                        ...
                        >+</v-btn>
                         <v-btn
                                 ...
                                 width="49"
                         >
                             <img @click="deleteProduct(index)" src="@/assets/plans/benefits/close.png" alt="close">
                         </v-btn>
                    </v-col>
                </v-row>
            </div>
        ...
        <div v-else>
            <h2 class="font__title mt-4 mb-16">No Products</h2>
        </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            // }),
            overalPrice() {
                return this.tmpCurrentPlan.products.reduce((sum, item) => {
        ...
        created() {
            let item = this.plansData.find(item => item.id === this.selectedPlanId);
            console.log(item);
        ...
        methods: {
            ...
            decrement(index) {
                let item = this.tmpCurrentPlan.products[index];
